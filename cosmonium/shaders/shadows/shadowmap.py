...
'''
    ...
    def prepare_shadow_for(self, code, light, light_direction, eye_light_direction):
        if self.use_bias:
            ...
            code.append(
                "vec4 %s_lightclip = trans_view_to_clip_of_%sLightSource * (eye_vertex4 + vec4(%s_offset, 0.0));"
                % (self.name, self.name, self.name)
            )
            code.append("%s_lightclip.z -= %s_shadow_depth_bias * %s_lightclip.w;" % (self.name, self.name, self.name))
        else:
            code.append(
                "vec4 %s_lightclip = trans_view_to_clip_of_%sLightSource * eye_vertex4;" % (self.name, self.name)
            )
        code.append(
            "%s_lightcoord = %s_lightclip * vec4(0.5, 0.5, 0.5, 1.0) + %s_lightclip.w * vec4(0.5, 0.5, 0.5, 0.0);"
            % (self.name, self.name, self.name)
        )
...
float shadow_pcf_16(sampler2DShadow shadow_map, vec4 shadow_coord)
{
    ...
    if (shadow_coord.w > .0)
    {
        ...
        for (y = -1.5 ; y <= 1.5 ; y += 1.0) {
            for (x = -1.5 ; x <= 1.5 ; x += 1.0) {
                ...
                shadow += textureProj(
                    shadow_map,
                    vec4(shadow_coord.x + offset_x, shadow_coord.y + offset_y, shadow_coord.z, shadow_coord.w));
            }
        }
        shadow /= 16.0;
    } else {
    ...
    def shadow_for(self, code, light, light_direction, eye_light_direction):
        if self.shader.fragment_shader.version < 130:
            code.append(
                "local_shadow *= 1.0 - (1.0 - shadow2D(%s_depthmap, %s_lightcoord.xyz).x) * %s_shadow_coef;"
                % (self.name, self.name, self.name)
            )
        else:
            if self.use_pcf_16:
                code.append(
                    "local_shadow *= 1.0 - (1.0 - shadow_pcf_16(%s_depthmap, %s_lightcoord)) * %s_shadow_coef;"
                    % (self.name, self.name, self.name)
                )
            else:
                code.append(
                    "local_shadow *= 1.0 - (1.0 - textureProj(%s_depthmap, %s_lightcoord)) * %s_shadow_coef;"
                    % (self.name, self.name, self.name)
                )
