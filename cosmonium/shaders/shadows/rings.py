...
class ShaderRingsShadow(ShaderComponent, ShaderShadowInterface):
    ...
    def shadow_for(self, code, light, light_direction, eye_light_direction):
        ...
        code.append(
            "float ring_intersection_param = "
            f"-dot(new_pos, ring_normal.xyz) / dot({light_direction}, ring_normal.xyz);"
        )
        ...
        code.append(
            '  float ring_shadow_local = '
            '(length(ring_intersection) - ring_inner_radius) / (ring_outer_radius - ring_inner_radius);'
        )
        code.append("  global_shadow *= 1.0 - texture(shadow_ring_tex, vec2(ring_shadow_local, 0.0)).a;")
        code.append("} else {")
