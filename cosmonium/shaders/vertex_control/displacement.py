...
class DisplacementVertexControl(VertexControl):
    ...
    def update_vertex(self, code):
        code.append(
            "vertex_height = %s;"
            % self.shader.data_source.get_source_for('height_%s' % self.heightmap.name, 'model_texcoord0.xy')
        )
    ...
    def update_normal(self, code):
        ...
        code.append(
            ...
            % self.shader.data_source.get_source_for('normal_%s' % self.heightmap.name, 'model_texcoord0.xy')
        )
