...
def convert_xy(
    ...
    """Convert and transform coordinate ranges.

    Transforms a 2D coordinate range by applying inversions and swaps.
    This is commonly used for texture coordinate transformations when
    generating geometry patches.
    ...
    Args:
        ...
        x_inverted: If True, inverts the X range (mirrors horizontally).
            Default is False.
        y_inverted: If True, inverts the Y range (mirrors vertically).
            Default is False.
        xy_swap: If True, swaps X and Y coordinates. Applied after
            inversions. Default is False.
    ...
    Returns:
        A tuple containing:
            ...
            - float: Delta X (x1 - x0 after transformations).
            - float: Delta Y (y1 - y0 after transformations).

    Notes:
        - Inversions map [x0, x1] to [1-x1, 1-x0].
        - Swapping exchanges X and Y axes.
        - Operations are applied in order: x_inverted, y_inverted, xy_swap.
    """
    if x_inverted:
        x0, x1 = 1 - x1, 1 - x0
    if y_inverted:
