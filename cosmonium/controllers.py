...
class BodyMover():
    ...
    def set_state(self, new_state):
        self.body.body.set_state(new_state)
...
class ShipMover(BodyMover):
    def set_local_position(self, position):
        self.body.set_frame_position(position)

    def get_local_position(self):
        return self.body.get_frame_position()

    def set_absolute_orientation(self, rotation):
        self.body.set_absolute_orientation(rotation)

    def get_absolute_orientation(self):
        return self.body.get_absolute_orientation()
    ...
    def step_relative(self, distance):
        rotation = self.body.get_frame_orientation()
    ...
    def turn(self, angle):
        ...
        self.set_absolute_orientation(new_rotation)

    def turn_relative(self, step):
        rotation = self.get_absolute_orientation()
        delta = LQuaterniond()
...
class ShipSurfaceBodyMover(ShipMover):
    ...
    def update(self):
        ...
        self.body.update(0, 0)

    def set_local_position(self, position):
        (x, y, z) = position
        new_frame_pos = LPoint3d(x, y, 0)
        self.body.set_frame_position(new_frame_pos)
        new_pos = self.body.get_local_position()
        distance = self.surface.get_height_under(new_pos)
        new_frame_pos[2] = distance + self.altitude
        self.body.set_frame_position(new_frame_pos)

    def get_position(self):
        pos = self.body.get_frame_position()
    ...
    def set_altitude(self, altitude):
        pos = self.body.get_local_position()
        distance = self.surface.get_height_under(pos)
        new_frame_pos = LPoint3d(self.body.get_frame_position())
    ...
    def step_relative(self, distance):
        rotation = self.body._frame_orientation
        direction = rotation.xform(LVector3d.forward())
        self.step(direction, distance)
