...
class FlatSurfaceParser(YamlModuleParser):
    ...
    def decode(cls, data):
        ...
        if heightmap_data is not None:
            heightmap = HeightmapYamlParser.decode(heightmap_data, 'heightmap', patched=True, scale=tile_size, coord_scale=coord_scale)
        else:
            heightmap = None
        if biome_data is not None:
            biome = HeightmapYamlParser.decode(biome_data, 'biome', patched=True, scale=tile_size, coord_scale=coord_scale)
        else:
            biome = None
