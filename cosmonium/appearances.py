...
class Appearance(AppearanceBase):
    ...
    def __init__(
        ...
        if texture is not None:
            self.set_texture(texture, tint, transparency, transparency_level, transparency_blend, context=context)
        if emission_texture is not None:
            self.set_emission_texture(emission_texture, tint, context=context)
        self.set_normal_map(normalMap, context=context)
        self.set_specular_map(specularMap, context=context)
        self.set_bump_map(bumpMap, bump_height, context=context)
