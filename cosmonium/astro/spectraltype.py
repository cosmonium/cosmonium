...
class SpectralTypeStringDecoder(object):
    ...
    def decode_subclass(self, name, length, i):
        subclass = None
        if i < length and name[i].isdigit():
            subclass = float(name[i])
            i += 1
            if i < length - 1 and name[i] == '.' and name[i + 1].isdigit():
                subclass += float(name[i + 1]) / 10
                i += 2
        return (subclass, i)

    def decode_main_class(self, name, length, i):
        spectral_class = None
        subclass = None
        if i < length:
            spectral_class = name[i]
            i += 1
            subclass, i = self.decode_subclass(name, length, i)
        return spectral_class, subclass, i

    def decode_second_class(self, name, length, i, spectral_class):
        second_spectral_class = None
        second_subclass = None
        if i < length - 1 and (name[i] == '/' or name[i] == '-'):
            i += 1
            ...
            second_subclass, i = self.decode_subclass(name, length, i)
        return (second_spectral_class, second_subclass, i)

    def decode_abundance(self, name, length, i):
        abundance = None
        if i < length - 1 and name[i] == ',' and name[i + 1].isdigit():
    ...
    def decode_luminosity(self, name, length, i):
        luminosity = None
        second_luminosity = None
        while i < length and name[i] == ' ':
            i += 1
        if i >= length or name[i] not in self.luminosities_first_char:
            return (luminosity, second_luminosity, i)
        for value in self.luminosities:
            ll = len(value)
            if i < length - ll + 1 and name[i : i + ll] == value:
                luminosity = value
                i += ll
                for sub_luminosity in self.sub_luminosities:
                    lsl = len(sub_luminosity)
                    if i < length - lsl + 1 and name[i : i + lsl] == sub_luminosity:
    ...
    def decode_peculiarity(self, name, length, i):
        while i < length and name[i] == ' ':
            i += 1
        if i >= length:
            return None, i
        ...
        for peculiarity in self.peculiarity_types:
            lp = len(peculiarity)
            if i < length - lp + 1 and name[i : i + lp] == peculiarity:
    ...
    def do_decode(self, spectral_type, name):
        ...
        if i < length - 1 and name[i : i + 2] in self.obsolete_main:
            ...
            if i < length:
                if name[i] in self.white_dwarf_classes:
                    spectral_type.spectral_class, spectral_type.subclass, i = self.decode_main_class(name, length, i)
                    spectral_type.peculiarity, i = self.decode_peculiarity(name, length, i)
                else:
                    print("unknown white dwarf type '%s'" % name)

        elif name[i] in self.mtw_prefix_1 or (i < length - 1 and name[i : i + 2] in self.mtw_prefix_2):
            spectral_type.main = True
            ...
            if i < length:
                if name[i] in self.main_spectral_classes:
                    spectral_type.spectral_class, spectral_type.subclass, i = self.decode_main_class(name, length, i)
                    spectral_type.second_spectral_class, spectral_type.second_subclass, i = self.decode_second_class(
                        name, length, i, spectral_type.spectral_class
                    )
                    spectral_type.peculiarity, i = self.decode_peculiarity(name, length, i)
                else:
            ...
            spectral_type.spectral_class, spectral_type.subclass, i = self.decode_main_class(name, length, i)
            spectral_type.abundance, i = self.decode_abundance(name, length, i)
            spectral_type.luminosity, spectral_type.second_luminosity, i = self.decode_luminosity(name, length, i)
            spectral_type.peculiarity, i = self.decode_peculiarity(name, length, i)
